...
document.getElementById('perPage').addEventListener('keypress', function (event) {
  if (event.key === 'Enter') {
    document.querySelector('.view-btn').click();
  }
});

document.getElementById('searchRepo').addEventListener('keypress', function (event) {
  if (event.key === 'Enter') {
...
function fetchUserAndRepositories() {
  ...
  var searchRepo = document.getElementById('searchRepo').value;
  if (searchRepo) {
    searchRepositories();
  } else {
...
function displayUserDetails(userDetails) {
  ...
  column1Div.append(`<img src="${userDetails.avatar_url}" alt="User Profile Image" class="img-fluid mb-3" style="border-radius: 50%; max-width: 150px;">`);
  column1Div.append(`<p><a href="${userDetails.html_url}" target="_blank">${userDetails.html_url}</a></p>`);
  ...
  column2Div.append(`<p><strong>📍</strong> ${userDetails.location || 'Not available'}</p>`);

  if (userDetails.twitter_username) {
    column2Div.append(`<p><i>Twitter:</i> <a href="https://twitter.com/${userDetails.twitter_username}" target="_blank">${userDetails.twitter_username}</a></p>`);
  }
  if (userDetails.blog) {
    column2Div.append(`<p><i>Blog:</i> <a href="${userDetails.blog}" target="_blank">${userDetails.blog}</a></p>`);
  }
...
function displayRepositories(repositories) {
  ...
  if (repositories.length === 0) {
    column1Div.append('<p class="text-muted">No repositories found.</p>');
    return;
  }
